...
your porters don't need to have it installed\.
.IP \(bu 2
You don't need to include minifiers in your package, reducing
the size for your porters\.
.IP \(bu 2
You don't need to rely on your porters having \fBcurl\fP or \fBwget\fP or
...
to the porter account or uid specified by the \fBuser\fP config, which
...
.RE
.P
then the porter could change the behavior by doing:
.P
.RS 2
.nf
...
the \fBnpm_config_binroot\fP environment variable is set to \fB/home/porter/bin\fP, then
don't try to install executables into \fB/usr/local/bin\fP\|\.  The porter
probably set it up that way for a reason\.
.IP \(bu 2
Don't prefix your script commands with "sudo"\.  If root permissions
are required for some reason, then it'll fail with that error, and
the porter will sudo the npm command in question\.
.IP \(bu 2
