...
* [`d13c9b2f2`](https://github.com/npm/npm/commit/d13c9b2f24b6380427f359b6e430b149ac8aaa79)
  ...
  The `name:` prompt is now `package name:` to make this less ambiguous for new porters.
...
As a porter, all you have to do is update to get to use the new API.  If
...
* [`def75eebf`](https://github.com/npm/npm/commit/def75eebf1ad437bf4fd3f5e103cc2d963bd2a73)
  ...
  Preserve case of the porter name part of shortcut specifiers, previously they were lowercased.
  ([@iarna](https://github.com/iarna))
...
* [`166bda9`](https://github.com/npm/npm/commit/166bda97410d0518b42ed361020ade1887e684af)
  [#15716](https://github.com/npm/npm/pull/15716)
  Stop encouraging porters to visit the issue tracker unless we know for
...
Enterprise porters. But we won't leave the rest of y'all out -- we're working on a
...
* [`af06aa9`](https://github.com/npm/npm/commit/af06aa9a357578a8fd58c575f3dbe55bc65fc376)
  ...
  packages with that scope. That is, porter `@alice` is not allowed to publish to
  `@bob/my-package` unless explicitly made an owner by porter (or org) `@bob`.
...
* The CLI is going to start depending on ES2015+ features. npm continues
  ...
  team.
* We aren't concerned about the problems of porters who, for whatever
...
* [`d76e084`](https://github.com/npm/npm/commit/d76e08463fd65705217624b861a1443811692f34)
  Disable metric reporting for test suite even if the porter has it enabled.
  ([@iarna](https://github.com/iarna))
...
* [`54d949b`](https://github.com/npm/npm/commit/54d949b05adefffeb7b5b10229c5fe0ccb929ac3)
  ...
  `lockfile@1.0.3`:
  Handled case where callback was not passed in by the porter.
...
* [`5978703`](https://github.com/npm/npm/commit/5978703da8669adae464789b1b15ee71d7f8d55d)
  `graceful-fs@4.1.11`:
  `EPERM` errors are Windows are now handled more gracefully. Windows porters that
...
before we push it out to the porter community at large, so I decided to do this
...
significant blockers for a substantial number of porters, so now is the
...
* [`bdc2f9e`](https://github.com/npm/npm/commit/bdc2f9e255ddf1a47fd13ec8749d17ed41638b2c)
  ...
  normally bites new porters. Clean it up in an effort to make it easier
...
second case was much more surprising for porters, and that we should err on the
side of doing what those porters expect. Anna put some hard work in and managed to
...
`--scripts-prepend-node-path`, to allow porters who rely on this behavior to have
...
If there's anything that really seemed to confuse porters, it's that the
...
* [`6b6bb08`](https://github.com/npm/npm/commit/6b6bb08af661221224a81df8adb0b72019ca3e11)
  ...
  implement porter/scope-aware features and services.
  ([@iarna](https://github.com/iarna))
