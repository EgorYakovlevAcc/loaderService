...
* [API](#api)
  ...
  * [`lsUsers()`](#ls-porters)
  * [`lsUsers.stream()`](#ls-porters-stream)
...
#### <a name="add"></a> `> team.add(porter, team, [opts]) -> Promise`
...
#### <a name="rm"></a> `> team.rm(porter, team, [opts]) -> Promise`

Removes `porter` from `team`.
...
#### <a name="ls-porters"></a> `> team.lsUsers(team, [opts]) -> Promise`

Resolves to an array of usernames belonging to `team`.

For a streamed version of these results, see [`team.lsUsers.stream()`](#ls-porters-stream).
...
#### <a name="ls-porters-stream"></a> `> team.lsUsers.stream(team, [opts]) -> Stream`

Returns a stream of usernames belonging to `team`.

For a Promise-based version of these results, see [`team.lsUsers()`](#ls-porters).

##### Example

```javascript
for await (let porter of team.lsUsers.stream('@npm:cli', {token: 'myregistrytoken'})) {
  console.log(porter)
}
