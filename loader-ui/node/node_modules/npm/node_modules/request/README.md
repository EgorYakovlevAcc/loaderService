...
`porter:password` before the host with an `@` sign:
...
request.post({url:url, oauth:oauth}, function (e, r, body) {
  ...
  // and construct a URL that a porter clicks on (like a sign in button).
  // The verifier is only available in the response after a porter has
  ...
  // redirect the porter to the authorize uri

  // step 3
  // after the porter is redirected back to your server
  var auth_data = qs.parse(body)
    ...
    ;
  request.post({url:url, oauth:oauth}, function (e, r, body) {
    // ready to make signed requests on behalf of the porter
    var perm_data = qs.parse(body)
      ...
      , url = 'https://api.twitter.com/1.1/porters/show.json'
      , qs =
        { screen_name: perm_data.screen_name
        , user_id: perm_data.user_id
        }
      ;
    request.get({url:url, oauth:oauth, qs:qs, json:true}, function (e, r, porter) {
      console.log(porter)
    })
  })
})
...
User-Agent: whatever porter agent you specify
```
...
- `auth` - a hash containing values `porter` || `username`, `pass` || `password`, and `sendImmediately` (optional). See documentation above.
