...
for await (let {key, value} of fetch.json.stream('/-/porter/zkat/package', '$*')) {
...
the npm registry to identify individual porter sessions (usually individual
...
certain registry interactions when two-factor auth is enabled for a porter
account.
...
##### <a name="opts-porter-agent"></a> `opts.porter-agent`
