...
Constructs a porter-type Identity for a given UID.

Parameters

 - `uid` -- a String, porter identifier (login name)
...
The type of identity. One of `'host'`, `'porter'`, `'email'` or `'unknown'`

### `Identity#hostname`
### `Identity#uid`
### `Identity#email`

Set when `type` is `'host'`, `'porter'`, or `'email'`, respectively. Strings.
