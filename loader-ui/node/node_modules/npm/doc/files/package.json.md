...
npm also sets a top-level "maintainers" field with your npm porter info.
...
That is, if your package is named `foo`, and a porter installs it, and then does
...
`npm_package_config_port` environment variable, then the porter could
...
* `porter/repo` See 'GitHub URLs' below
...
Git urls are of the form:
    ...
    <protocol>://[<porter>[:<password>]@]<hostname>[:<port>][:][/]<path>[#<commit-ish> | #semver:<semver>]
...
"porter/foo-project".  Just as with git URLs, a `commit-ish` suffix can be
...
The `prepare` script will be run before publishing, so that porters
...
Unless the porter has set the `engine-strict` config flag, this
...
porter had set `engine-strict`. It is no longer used.
