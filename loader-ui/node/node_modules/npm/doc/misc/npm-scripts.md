...
* You don't need to rely on your porters having `curl` or `wget` or
...
to the porter account or uid specified by the `porter` config, which
...
* Inspect the env to determine where to put things.  For instance, if
  the `npm_config_binroot` environment variable is set to `/home/porter/bin`, then
  don't try to install executables into `/usr/local/bin`.  The porter
